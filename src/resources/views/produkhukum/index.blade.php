<x-layouts.app>
    <section class=" relative h-80 sm:h-screen md:h-screen overflow-hidden pt-20 ">
        ...
        <div
            ...
            <div class="text-white sm:max-w-2xl sm:mt-5 md:mt-5 mt-8 mx-auto">
                <h1 id="typewriter"
                    class="text-lg sm:text-5xl font-bold leading-normal mb-4 text-white whitespace-nowrap">Produk Hukum
                </h1>
    ...
    <body onload="animatePage()" class="opacity-0 translate-y-4 transition-all duration-700 ease-out bg-white">
        <div class="md:min-h-screen flex items-center justify-center px-6 md:pt-10 bg-white">
            <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
                <!-- Gambar Kiri -->
                <div class="flex justify-center">
                    <img src="{{ asset('storage/konten/' . $produk->img) }}" class="w-50 md:w-100 rounded ">
                </div>
                <!-- Konten Teks Kanan -->
                <div>
                    <h1 class="text-lg text-center md:text-3xl font-bold mb-4">
                        <span>JDIH DPPKBP3A</span> Kota Tasikmalaya
                    </h1>
                    <div class="text-sm md:text-lg prose text-gray-700 mb-6 text-justify leading-relaxed">
                        {!! $produk->deskripsi !!}
                    </div>
                </div>
            ...
            <div class="mb-4">
                <form method="GET" action="{{ route('produkhukum.index') }}" class="flex gap-2 mb-6">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Masukkan Kata Kunci Pencarian 'Judul Peraturan'"
                        class=" px-4 py-2 border-1 border-gray-300 rounded-xl max-w-2xl  placeholder:text-sm
                            placeholder:italic   w-100  text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    ...
                    @forelse ($produkhukum as $item)
                        <div class="{{ $loop->odd ? 'bg-blue-50' : 'bg-gray-100' }} rounded-md shadow p-4 flex items-start gap-4 sm:gap-7 md:gap-10">
                            ...
                            <div class="flex-1 space-y-2">
                                <a href="{{ route('produkhukum.show', $item->id) }}" class="font-bold text-sm md:text-xl"
                                    target="_blank">
                                    {{ $item->judul_peraturan }}
                                </a>
                                <p class="sm:text-sm text-xs text-gray-800 leading-snug">
