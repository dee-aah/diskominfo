<x-layouts.app>
<section class="relative h-screen overflow-hidden pt-16"> <!-- tambahkan pt-16 untuk kompensasi navbar -->
  <div><img class="absolute top-0 left-0 w-full h-full object-cover object-top z-0 transform -translate-y-5"src="{{ asset('img/gambar.jpg') }}" alt="">
</div>

    <!-- Overlay -->
    <div class="absolute bg-black bg-opacity-50 z-10"></div>

    <!-- Konten Hero -->
 <div class="relative z-20 flex items-center justify-center min-h-screen text-center px-4 pt-8 pb-8">
      <div class="text-white max-w-2xl">
       <h1 id="typewriter" class="text-4xl md:text-5xl font-bold leading-normal mb-4 text-white whitespace-nowrap">Tupoksi</h1>

        <p class="text-lg mb-6">Dinas Pengendalian Penduduk, Keluarga Berencana, Pemberdayaan Perempuan, dan Perlindungan Anak.</p>

      </div>
    </div>

    <!-- Wave SVG -->
    <div class="absolute bottom-0 left-0 right-0 overflow-hidden leading-[0] z-20">
      <svg class="relative block w-full h-[100px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" preserveAspectRatio="none">
        <path fill="#E2E7F6" fill-opacity="1" d="M0,128L48,154.7C96,181,192,235,288,234.7C384,235,480,181,576,149.3C672,117,768,107,864,122.7C960,139,1056,181,1152,192C1248,203,1344,181,1392,170.7L1440,160L1440,320L0,320Z"></path>
      </svg>
    </div>
  </section>
   <div class="bg-indigo-100 justify-center">
    <h2 class="text-center text-lg font-semibold text-gray-800 mb-2">
      Tugas Pokok dan Fungsi Organisasi
    </h2>
    <p class="text-center text-sm text-gray-600 mb-8">
      Tugas Pokok dan Fungsi merupakan pedoman peran strategis dalam mendukung tercapainya visi organisasi.
      Setiap elemen dalam Tupoksi mencerminkan kontribusi nyata terhadap efisiensi, pelayanan, dan pengembangan berkelanjutan.
    </p>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
      <!-- Tugas -->
      <div class="bg-gray-100 rounded-lg p-6 text-center rounded-xl">
        <div class="flex justify-center mb-4">
          <div class="bg-indigo-300 p-3 rounded-full">
            <!-- Ikon tugas -->
            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" stroke-width="2"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M9 17v-2a2 2 0 012-2h2a2 2 0 012 2v2m-6 0h6m2 0a2 2 0 002-2V7a2 2 0 00-2-2H6a2 2 0 00-2 2v8a2 2 0 002 2h2"/>
            </svg>
          </div>
        </div>
        <h3 class="font-semibold text-gray-800 text-lg underline mb-2">Tugas</h3>
        <p class="text-sm text-gray-700">
          Kepala Dinas mempunyai tugas pokok membantu Bupati melaksanakan urusan pemerintahan yang menjadi kewenangan daerah dan tugas pembantuan di bidang Pengendalian Penduduk, Keluarga Berencana, Pemberdayaan Perempuan dan Perlindungan Anak
        </p>
      </div>

      <!-- Fungsi Utama -->
      <div class="bg-gray-100 rounded-lg p-6 text-center rounded-xl">
        <div class="flex justify-center mb-4">
          <div class="bg-gray-400 p-3 rounded-full">
            <!-- Ikon fungsi -->
            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" stroke-width="2"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M12 4v16m8-8H4"/>
            </svg>
          </div>
        </div>
        <h3 class="font-semibold text-gray-800 text-lg underline mb-2">Fungsi Utama</h3>
        <ol class="text-sm text-gray-700 text-left list-decimal list-inside space-y-1">
          <li>Perumusan dan Pelaksanaan Kebijakan Daerah di bidang : Kesekretariatan, Pengendalian Penduduk dan KB, Advokasi dan Bina Lini Lapangan, Pemberdayaan Perempuan dan Perlindungan Anak.</li>
          <li>Koordinasi dan Fasilitasi</li>
          <li>Monitoring dan Evaluasi</li>
          <li>Pembinaan terhadap UPTD dan Tenaga Penyuluh KB</li>
          <li>Pelayanan Administrasi dan Pengelolaan Alat Kontrasepsi</li>
        </ol>
      </div>
    </div>
  </div>
  <div class="bg-indigo-100 pb-8" >
<div class="bg-white rounded-t-3xl p-6 w-full max-w-screen-xl mx-auto ">
    <div class="flex flex-col items-center mb-6">
      <!-- Ikon Folder -->
      <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 text-yellow-400 mb-2" viewBox="0 0 24 24" fill="currentColor">
        <path d="M10 4H4a2 2 0 00-2 2v1h20V8a2 2 0 00-2-2h-8l-2-2z"/>
        <path d="M2 9v9a2 2 0 002 2h16a2 2 0 002-2V9H2z"/>
      </svg>
      <h2 class="text-lg font-semibold text-gray-800 border-b border-gray-300 pb-1">Uraian Tugas</h2>
    </div>

    <!-- Kepala Dinas -->
    <div class="mb-6">
      <div class="flex items-center space-x-2 mb-1">
        <!-- Ikon Kepala Dinas -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 12c2.67 0 8 1.34 8 4v3H4v-3c0-2.66 5.33-4 8-4zm0-2a4 4 0 100-8 4 4 0 000 8z"/>
        </svg>
        <h3 class="font-bold text-gray-800">Kepala Dinas</h3>
      </div>
      <ol class="list-decimal list-inside text-gray-700 text-sm ml-6">
        <li>Merumuskan Kebijakan Strategis</li>
        <li>Mengkoordinasikan Seluruh Bidang dan Sekretariat</li>
        <li>Memberikan saran Kepada Kepala Daerah</li>
        <li>Menyusun Laporan Pelaksanaan Tugas</li>
      </ol>
    </div>

    <!-- Sekretariat -->
    <div>
      <div class="flex items-center space-x-2 mb-1">
        <!-- Ikon Sekretariat -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 12c2.67 0 8 1.34 8 4v2H4v-2c0-2.66 5.33-4 8-4zm0-2a4 4 0 100-8 4 4 0 000 8z"/>
        </svg>
        <h3 class="font-bold text-gray-800">Sekretariat</h3>
      </div>
      <ol class="list-decimal list-inside text-gray-700 text-sm ml-6">
        <li>Mengelola Administrasi Umum, Kepegawaian, Keuangan dan Perencanaan</li>
        <li>Menyusun Program Kerja dan Laporan Kegiatan</li>
      </ol>
    </div>
    <br>
    <div>
      <div class="flex items-center space-x-2 mb-1">
        <!-- Ikon Sekretariat -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 12c2.67 0 8 1.34 8 4v2H4v-2c0-2.66 5.33-4 8-4zm0-2a4 4 0 100-8 4 4 0 000 8z"/>
        </svg>
        <h3 class="font-bold text-gray-800">Bidang Pengendalian Penduduk</h3>
      </div>
      <ol class="list-decimal list-inside text-gray-700 text-sm ml-6">
        <li>Advokasi dan Penggerakan Mayarakat</li>
        <li>Penyuluhan dan Pendayagunaan Petugas KB</li>
        <li>Pengumpulan dan Analisis Data Kependudukan</li>
      </ol>
    </div>
    <br>
    <div>
      <div class="flex items-center space-x-2 mb-1">
        <!-- Ikon Sekretariat -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 12c2.67 0 8 1.34 8 4v2H4v-2c0-2.66 5.33-4 8-4zm0-2a4 4 0 100-8 4 4 0 000 8z"/>
        </svg>
        <h3 class="font-bold text-gray-800">Bidang Keluarga Berencana</h3>
      </div>
      <ol class="list-decimal list-inside text-gray-700 text-sm ml-6">
        <li>Distribusi Alat dan Obat Kontrasepsi</li>
        <li>Jaminan Pelayanan KB</li>
        <li>Pembinaan KB</li>
      </ol>
    </div>
    <br>
    <div>
      <div class="flex items-center space-x-2 mb-1">
        <!-- Ikon Sekretariat -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-black" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 12c2.67 0 8 1.34 8 4v2H4v-2c0-2.66 5.33-4 8-4zm0-2a4 4 0 100-8 4 4 0 000 8z"/>
        </svg>
        <h3 class="font-bold text-gray-800">Bidang Pemberdayaan Perempuan dan Perlindungan Anak</h3>
      </div>
      <ol class="list-decimal list-inside text-gray-700 text-sm ml-6">
        <li>Pengarusutamaan Gender dan Hak Anak</li>
        <li>Perlindungan Permpuan dan Anak dari Kekerasan</li>
        <li>Pembinaan Organisasi Perempuan</li>
      </ol>
    </div>
  </div>
  </div>
  </body>
</x-layouts.app>
