        ...
        <div class="rounded-2xl border  border-gray-200 bg-white ">
            <div class="flex  p-4 justify-center items-center ">
                <div class="flex  items-center ">
                    <h3 class="text-2xl text-center font-medium">Edit Dok Perencanaan</h3>
                </div>
            </div>
            <div id="main-content" class="p-5 border-t border-gray-300 sm:p-6 ">
                ...
                <form action="{{ route('perencanaan.update', $perencanaan->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 text-lg font-medium md:grid-cols-2 gap-6"> Konten
                    <div class="my-3">
                        <label class="block py-2 text-[15px] text-black font-medium">Deskripsi Singkat</label>
                        <textarea name="des_singkat" rows="5"
                            placeholder="Masukkan Deskripsi Program"
                            class="w-full border editor  border-gray-300 placeholder:text-sm text-sm rounded-lg p-2" required>{{ old('des_singkat', $perencanaan->des_singkat) }}</textarea>
                    </div>
                    <div class="my-3">
                        <label class="block py-2 text-[15px] text-black font-medium">Foto Konten</label>
                        <input  type="file" name="img_konten"
                            ...
                            <img src="{{ asset('storage/perencanaan/' . $perencanaan->img_konten) }}" class="w-32 mt-1 rounded">
                        @endif
                    </div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> Dokumen
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input  type="file" name="img_pdf"
                            ...
                            <p class="mt-2 text-sm text-gray-500">Gambar saat ini:</p>
                            <img src="{{ asset('storage/perencanaan/' . $perencanaan->img_pdf) }}" class="w-32 mt-1 rounded">
                        @endif
                    </div>
                    </div>
