        ...
        <div class="rounded-2xl border  border-gray-200 bg-white ">
            <div class="flex justify-between px-3 grid grid-cols-3 items-center ">
                <div class="justify-start ml-4 col-1">
                    <h2 class="text-xl  font-medium">Struktur Organisasi</h2>
                </div>
                <div class="col-span-2 flex py-5 justify-end">
                    <form action="{{ route('strukturr.dashboard') }}" method="GET" class="flex justify-end  mr-2">
                    ...
                    <a href="{{ route('strukturr.create') }}" type="button"
            ...
            <thead>
                <tr class="border-gray-200 border-b">
                    ...
                    <th class="font-medium border-r  border-gray-200 px-3 py-3">Gambar Waktu Dibuat</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($strukturs as $struktur)
                    <tr class="text-center mx-3 items-center hover:bg-gray-100">
                        <td class="p-3 border-r  border-gray-200 text-center">
                            @if ($struktur->img)
                                <img src="{{ asset('storage/struktur/' . $struktur->img) }}"
                        ...
                        <td class="px-3 border-r  border-gray-200 py-3">
                                            {{ $struktur->created_at->translatedFormat('l d F Y.') }}</td>
                                        <td class="p-3  align-middle relative z-10">
                                            <el-dropdown class="inline-block">
                                                ...
                                                <el-menu anchor="bottom end" popover
                                                    ...
                                                    <div class="py-1">
                                                        <a href="{{ route('strukturr.show', $struktur) }}"
                                                        ...
                                                        <a href="{{ route('strukturr.edit', $struktur) }}"
                                                            class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-300">
                                                            <i class="fa-solid fa-pen text-gray-600 px-3"></i> Edit
                                                        </a>
                                                        <form action="{{ route('strukturr.destroy', $struktur) }}"
                                                            method="POST"
                                                            onsubmit="return confirm('Yakin hapus struktur ini?')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-3 text-center py-3">Belum ada Struktur Organisasi</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
