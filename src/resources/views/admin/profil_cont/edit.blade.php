    ...
    <div class="max-w-5xl mx-auto  min-h-screen ml-2">
        <div class="rounded-2xl border  border-gray-200 bg-white ">
            ...
            <div id="main-content" class="p-5 border-t border-gray-300 sm:p-6 ">
                ...
                <form action="{{ route('profil_cont.update', $profil->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="my-3">
                        ...
                        <textarea name="deskripsi" rows="5"
                            ...
                            class="w-full border editor border-gray-300 placeholder:text-sm text-sm rounded-lg p-2" required>{{ old('deskripsi', $profil->deskripsi) }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label class="block py-2 text-[15px] text-black font-medium">Gambar Latar</label>
                        <input  type="file" name="gambar" id="editor"
                            ...
                            <img src="{{ asset('storage/profil_cont/' . $profil->gambar) }}" class="w-32 mt-1 rounded">
                        @endif
                    </div>

                    <div class="flex justify-end gap-2 mt-4">
                        <a href="{{ route('profil_cont.dashboard') }}"
