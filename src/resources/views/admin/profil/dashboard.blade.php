    ...
    <div class="rounded-2xl border  border-gray-200 bg-white ">
            <div class="flex justify-between px-3 grid grid-cols-3 items-center ">
                <div class="justify-start ml-4 col-1">
                    <h2 class="text-xl  font-medium">Profil Pimpinan</h2>
                </div>
                <div class="col-span-2 flex py-5 justify-end">
                    <form action="{{ route('profil.dashboard') }}" method="GET" class="flex justify-end  mr-2">
        ...
        <table class="table-auto wrapper min-w-full text-sm">
            <thead class="border-b border-gray-200">
                <tr class="border-gray-200 border-b">
                    <th class="font-medium border-r  border-gray-200 px-3 py-3">Foto Pimpinan</th>
                    <th class="font-medium border-r  border-gray-200 px-3 py-3">Nama Pimpinan</th>
                    <th class="font-medium border-r  border-gray-200 px-3 py-3">Jabatan</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($profils as $profil)
                    <tr class="text-center mx-3 items-center hover:bg-gray-100">
                        <td class="px-3 border-r  border-gray-200 py-3 text-center">
                            @if ($profil->img)
                                <img src="{{ asset('storage/profil/' . $profil->img) }}"
                                    class="w-16 h-16 object-cover mx-auto justify-content-center rounded">
                            @else
                                -
                            @endif
                        </td>
                        <td class="px-3 border-r  border-gray-200  py-3">{{ $profil->nama }}</td>
                        <td class="px-3 border-r  border-gray-200 py-3">{{ $profil->jabatan }}</td>
                        <td class="px-3 border-r  border-gray-200 py-3">{{ $profil->created_at->translatedFormat('l d F Y.') }}</td>
                        <td class="p-3  align-middle relative z-10">
                                            <el-dropdown class="inline-block">
                                                ...
                                                <el-menu anchor="bottom end" popover
                                                    ...
                                                    <div class="py-1">
                                                        <a href="{{ route('profil.show', $profil) }}"
                                                        ...
                                                        <a href="{{ route('profil.edit', $profil) }}"
                                                            class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-300">
                                                            <i class="fa-solid fa-pen text-gray-600 px-3"></i> Edit
                                                        </a>
                                                        <form action="{{ route('profil.destroy', $profil) }}"
                                                            method="POST"
                                                            onsubmit="return confirm('Yakin hapus Profil pimpinan ini?')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-3 text-center py-3">Belum ada Profil Pimpinan</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
