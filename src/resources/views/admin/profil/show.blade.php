        ...
        <div  class="rounded-2xl border  border-gray-200 bg-white ">
            <div class=" justify-between px-3  items-center ">
                ...
                <div class="flex text-[13px] p-5 text-gray-500justify-start">
                    <a href="{{ route('profil.dashboard') }}" class="inline-block mb-2  hover:underline">
                        Dashboard
                    </a>
                ...
                {{-- Judul artikel --}}
                <h1 class="text-[30px] text-blue-500 text-center font-bold mb-4">{{ $profil->nama }}</h1>
                <p class="text-gray-500 text-center text-sm mb-3">
                    Dipublikasikan Pada {{ $profil->created_at->translatedFormat('l d F Y.') }}
                </p>

                {{-- Gambar artikel --}}
                @if ($profil->img)
                    <div class="mb-6">
                        <img src="{{ asset('storage/profil/' . $profil->img) }}" alt="{{ $profil->nama }}"
