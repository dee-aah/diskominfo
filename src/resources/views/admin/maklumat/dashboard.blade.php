...
<div class="rounded-2xl border  border-gray-200 bg-white ">
            <div class="flex justify-between px-3 grid grid-cols-3 items-center ">
                <div class="justify-start ml-4 col-1">
                    <h2 class="text-xl  font-medium">Maklumat</h2>
                </div>
                <div class="col-span-2 flex py-5 justify-end">
                    <form action="{{ route('maklumat.dashboard') }}" method="GET" class="flex justify-end  mr-2">
                    ...
                    <a href="{{ route('maklumat.create') }}" type="button"
            ...
            <thead class="border-b border-gray-200">
                <tr class="border-gray-200 border-b">
                    ...
                    <th class="font-medium border-r  border-gray-200 p-3">Gambar latar</th>
                    <th class="font-medium p-3">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
                @forelse ($maklumats as $maklumat)
                    <tr class="text-center mx-3 items-center hover:bg-gray-100">
                        <td class="p-3 border-r  border-gray-200 ">{{ $maklumat->deskripsi }}</td>
                        <td class="p-3 border-r  border-gray-200 text-center">
                            @if ($maklumat->gambar_cont)
                                <img src="{{ asset('storage/maklumat1/' . $maklumat->gambar_cont) }}"
                        ...
                        <td class="p-3 border-r  border-gray-200 text-center">
                            @if ($maklumat->gambar)
                                <img src="{{ asset('storage/maklumat1/' . $maklumat->gambar) }}"
                                    class="w-16 h-16 object-cover mx-auto justify-content-center rounded">
                            @else
                                -
                            @endif
                        ...
                        <td class="p-3  align-middle ">
                            ...
                            <a href="{{ route('maklumat.edit', $maklumat->id) }}"
                                class=" col-span-2  p-3  ">
                                    <i class="fa-solid fa-pen text-gray-600 hover:text-yellow-500 cursor-pointer"></i>
                            </a>
                            <form action="{{ route('maklumat.destroy', $maklumat->id) }}" method="POST"
                                onsubmit="return confirm(' Anda Yakin Ingin Menghapus Maklumat ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class=" col-span-2 p-3  ">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-3">Belum ada Maklumat</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
