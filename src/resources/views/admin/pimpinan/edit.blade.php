    ...
    <main>
            <div class="max-w-3xl mx-auto">
                <div class="flex justify-center items-center mb-4">
                    <h3 class="text-3xl text-center font-bold">Edit Beranda</h3>
                </div>
                <form action="{{ route('pimpinan.update', $pimpinan->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="deskripsi" rows="5"
                            ...
                            class="w-full border bg-gray-100 border-gray-400 border-2 rounded p-2" required>{{ old('deskripsi', $pimpinan->deskripsi) }}</textarea>
                    </div>
                    <div class="flex ">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                            <label class="block text-lg font-medium">Vidio Latar  </label>
                        <input  type="file" name="vidio"
                            class="w-full p-2 justify-end file:mr-4 file:rounded-3xl file:border-0 file:bg-violet-50
                            file:px-4 file:py-2 file:text-sm  file:font-semibold file:text-blue-600 hover:file:bg-violet-100 dark:file:bg-blue-600 dark:file:text-violet-100 dark:hover:file:bg-violet-400 ..." />
                        @if ($pimpinan->vidio)
                            <p class="mt-2 text-sm text-gray-500">Gambar saat ini:</p>
                            <img src="{{ asset('storage/pimpinan/' . $pimpinan->vidio) }}" class="w-32 mt-1  rounded">
                        @endif
                    </div>
                    </div>
