    ...
    <div class="p-6">
        <div class="flex justify-between grid grid-cols-3 items-center mb-4">
            <div class="justify-start col-1">
                <h2 class="text-2xl  font-bold">Daftar Uraian</h2>
            </div>
            <div class="col-span-2 flex justify-end">
                <form action="{{ route('uraian.dashboard') }}" method="GET" class="flex justify-end  mr-2">
                    <input type="text" name="d" value="{{ request('d') }}" placeholder="Cari Uraian..."
        ...
        <table class="w-full table-auto border-gray-400 border-2 border text-sm">
            <thead>
                <tr class="bg-slate-300 text-center">
                    ...
                    <th class="px-4 border border-gray-400 border-2 py-2">Bidang</th>
                    <th class="px-4 border border-gray-400 border-2 py-2">Uraian</th>
            ...
            <tbody class="bg-gray-100">
                @forelse ($uraians as $uraian)
                    <tr class="text-center items-center ">
                        ...
                        <td class="px-4 border border-gray-400 border-2 py-2">{{ $uraian->bidang }}</td>
                        <td class="px-4 border border-gray-400 border-2 py-2">{{ $uraian->uraian }}</td>
                        <td class="px-3 py-3 flex grid grip-rows border border-gray-400 border-2 justify-center aksi hidden">
                            <a href="{{ route('uraian.edit', $uraian->id) }}"
                            ...
                            <form action="{{ route('uraian.destroy', $uraian->id) }}" method="POST"
                                onsubmit="return confirm(' Anda Yakin Ingin Menghapus Uraian ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center border border-gray-400 border-2 py-4">Belum ada Uraian</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
