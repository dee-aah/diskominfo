    ...
    <div class="max-w-5xl mx-auto flex-1  min-h-screen ml-2 ">
        @if (session('success'))
            <div id="flash-message"
                class="bg-green-100 border border-green-400 text-black flex justify-between max-w-5xl text-center  p-3 rounded mb-4">
                <div>
                </div>
                <div>
                    {{ session('success') }}
        ...
        <div class="rounded-2xl border  border-gray-200 bg-white ">
            <div class="flex justify-between px-3 grid grid-cols-3 items-center ">
                <div class="justify-start ml-4 col-1">
                    <h2 class="text-xl  font-medium">Uraian Tugas</h2>
                </div>
                <div class="col-span-2 flex py-5 justify-end">
                    <form action="{{ route('uraian.dashboard') }}" method="GET" class="flex justify-end  mr-2">
                        ...
                        <a href="{{ route('uraian.create') }}" type="button"
                        ...
                        <table class="table-auto wrapper min-w-full text-sm">
                            <thead>
                                <tr class="border-gray-200 border-b">
                                    <th class="font-medium border-r  border-gray-200 p-3">Bidang</th>
                                    <th class="font-medium border-r  border-gray-200 p-3">Uraian</th>
                                    <th class="font-medium border-r  border-gray-200 p-3">Waktu Dibuat</th>
                                    <th class="font-medium p-3">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($uraians as $uraian)
                                    <tr class="text-center mx-3 items-center hover:bg-gray-100">
                                        <td class="p-3 border-r  border-gray-200">{{ $uraian->bidang }}</td>
                                        <td class="p-3 border-r  border-gray-200">{!! Str::limit($uraian->uraian, 50) !!}</td>
                                        <td class="px-3 border-r  border-gray-200 py-3">
                                            {{ $uraian->created_at->translatedFormat('l d F Y.') }}</td>
                                        <td class="p-3  align-middle relative z-10">
                                            <el-dropdown class="inline-block">
                                                ...
                                                <el-menu anchor="bottom end" popover
                                                    ...
                                                    <div class="py-1">
                                                        <a href="{{ route('uraian.show', $uraian) }}"
                                                        ...
                                                        <a href="{{ route('uraian.edit', $uraian) }}"
                                                            class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-300">
                                                            <i class="fa-solid fa-pen text-gray-600 px-3"></i> Edit
                                                        </a>
                                                        <form action="{{ route('uraian.destroy', $uraian) }}"
                                                            method="POST"
                                                            onsubmit="return confirm('Yakin hapus Uraian ini?')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="p-3 text-center">Belum ada Uraian</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
